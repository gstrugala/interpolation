...
def F_L(point):
    with open('permap.txt', 'r') as f:
        ...
        f.readline()
        locs = np.array([find(feature, value) for feature, value in zip(features, target)])
        norm_point = tuple((pt_coord - feature[loc]) / (feature[loc+1] - feature[loc])
                           for feature, pt_coord, loc in zip(features, point, locs))
        ord_point_idx = np.argsort(norm_point)  # find Fortran function to sort and get permutations
        s = [np.zeros_like(norm_point, dtype=int)]
        for i, idx in enumerate(ord_point_idx[::-1]):
            s2 = s[i].copy()
            s2[idx] = 1
            s.append(s2)
        vertex_locs = [s + locs for s in s]
        nf_products = np.append(nfeatures[1:], 1)
        for i in range(len(nf_products) - 2, -1, -1):
            nf_products[i] *= nf_products[i + 1]
        lines = [np.dot(vertex_loc, nf_products) for vertex_loc in vertex_locs]
        performance_data = np.zeros((len(nf_products)+1, 3))
        ...
        for i, nlines in enumerate(np.insert(np.diff(lines), 0, 1)):
            for _ in range(nlines - 1):
                f.readline()
            performance_data[i, :] = np.array(f.readline().split()[5:]).astype('float')
        for vertex_loc, pdat in zip(vertex_locs, performance_data):
            print([(i, feature[i]) for i, feature in zip(vertex_loc, features)], pdat)
        FL = performance_data[-1].copy()
        p_dat_old = performance_data[-1].copy()
        for i, p_dat in enumerate(performance_data[:-1][::-1]):
            print(norm_point[ord_point_idx[i]], p_dat)
            FL +=  (1 - norm_point[ord_point_idx[i]]) * (p_dat - p_dat_old)
            p_dat_old = p_dat
